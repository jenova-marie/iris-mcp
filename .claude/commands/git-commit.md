...
- Filter to include only `src/` folder changes (as per unit-test agent requirements)

#### 6. Invoke Unit Test Agent
- Pass the filtered git changes to the `unit-test` agent
...
If the unit-test agent made changes:
...
#### 8. Invoke Tech Writer Agent  
- Pass the original git changes (from step 5) to the `tech-writer` agent
...
If the tech-writer agent made changes:
- Review the changes made by the agent  
...
- If unit-test agent fails, continue with documentation phase
- If tech-writer agent fails, report the issue but don't block
...
### Test Updates
- Files modified by unit-test agent
- Test commit hash if created
- Summary of REQUESTED.md updates

### Documentation Updates  
- Files modified by tech-writer agent
