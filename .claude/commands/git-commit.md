...
- Pass the original git changes (from step 5) to the `ms-tech-writer` agent
...
If the ms-tech-writer agent made changes:
- Review the changes made by the agent
- Create commit(s) for documentation updates:
  ...
  Generated by: ms-tech-writer agent
  ```
...
- If ms-tech-writer agent fails, report the issue but don't block
...
- Files modified by ms-tech-writer agent
