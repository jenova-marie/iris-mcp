...
export function ProcessMonitor() {
  ...
  const [terminalStatus, setTerminalStatus] = useState<{
    [sessionId: string]: "idle" | "launching" | "success" | "copied";
  }>({});
  ...
  const handleViewCache = async (
    ...
    try {
      ...
      setCacheData((prev) => ({
        ...prev,
        [sessionId]: [
          `Error loading cache: ${error instanceof Error ? error.message : String(error)}`,
        ],
      }));
    }
  };
  ...
  const handleSleep = useCallback(
    async (fromTeam: string, toTeam: string, poolKey: string) => {
      ...
      try {
        ...
        alert(
          `Failed to sleep session: ${error.response?.data?.error || error.message}`,
        );
  ...
  const handleClear = useCallback(
    async (fromTeam: string, toTeam: string, poolKey: string) => {
      if (
        !confirm(
          `Clear session ${poolKey}? This will terminate the process, delete the old session, and create a fresh new one.`,
        )
      ) {
        return;
      }
      ...
      try {
        await api.rebootSession(fromTeam, toTeam);
        setActionStatus((prev) => ({ ...prev, [poolKey]: "success" }));
        queryClient.invalidateQueries({ queryKey: ["sessions"] });
        ...
        setActionStatus((prev) => ({ ...prev, [poolKey]: "error" }));
        alert(
          `Failed to clear session: ${error.response?.data?.error || error.message}`,
        );
  ...
  const handleDelete = useCallback(
    async (fromTeam: string, toTeam: string, poolKey: string) => {
      if (
        !confirm(
          `Delete session ${poolKey}? This will permanently remove the session data. This cannot be undone.`,
        )
      ) {
        return;
      }
      ...
      try {
        ...
        setActionStatus((prev) => ({ ...prev, [poolKey]: "error" }));
        alert(
          `Failed to delete session: ${error.response?.data?.error || error.message}`,
        );
          ...
          <div className="flex items-center gap-4">
            <div className="text-right">
              ...
              <p className="text-2xl font-bold text-accent-purple">
                {poolStatus.activeProcesses || 0} /{" "}
                {poolStatus.maxProcesses || 0}
              </p>
            </div>
            <div className="text-right">
              ...
              <div className="flex items-center gap-2 mt-1">
                <div
                  className={`w-2 h-2 rounded-full ${connected ? "bg-status-idle" : "bg-status-offline"}`}
                />
            ...
            <div key={session.poolKey} className="card card-hover">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div
                    className="relative"
                    ref={openDropdown === session.poolKey ? dropdownRef : null}
                  >
                    <button
                      onClick={() =>
                        setOpenDropdown(
                    ...
                    {openDropdown === session.poolKey && (
                      <div className="absolute right-0 mt-2 w-48 bg-bg-card border border-gray-700 rounded-lg shadow-lg z-10">
                        <button
                          ...
                          disabled={
                            actionStatus[session.poolKey] !== "idle" &&
                            actionStatus[session.poolKey] !== undefined
                          }
