 ...
 * - tell() requires fromTeam parameter
 * - report() requires fromTeam parameter
...
const logger = getChildLogger("test:output-cache");

describe("Output Cache with Async Messages (New Architecture)", () => {
      ...
      const syncResult = await tell(
        {
          ...
          fromTeam: "team-iris", // External request
  ...
  it(
    ...
    async () => {
      ...
      }, "Cache after multiple async messages");
  ...
  it(
    "should preserve cache across multiple messages",
    async () => {
      logger.info("Testing cache preservation across messages");
      ...
      }, "Combined output from multiple messages");
