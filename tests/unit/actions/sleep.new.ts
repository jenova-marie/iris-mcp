...
describe("sleep", () => {
  ...
  describe("team already asleep", () => {
    it("should return already_asleep status", async () => {
      ...
      const result = await sleep(
        { team: "team-alpha", fromTeam: "external" },
        mockProcessPool
      );
  ...
  describe("putting awake team to sleep", () => {
    ...
    it("should include lost messages when force=true", async () => {
      const result = await sleep(
        { team: "team-alpha", fromTeam: "external", force: true },
        mockProcessPool
      );
    ...
    it("should handle termination errors", async () => {
      ...
      expect(result.message).toContain("Failed to put team team-alpha to sleep");
    });
  });
  ...
  describe("validation", () => {
    ...
    it("should throw ConfigurationError for unknown team", async () => {
      await expect(
        sleep(
          { team: "unknown-team", fromTeam: "external" },
          mockProcessPool
        )
